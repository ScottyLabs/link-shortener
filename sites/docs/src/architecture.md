...
1. The user clicks "Log in" on the frontend.
1. The browser navigates to a protected route such as `/api/me`, which is behind `OidcLoginLayer`.
1. axum-oidc redirects the browser to Keycloak. The redirect URI registered with the client is the shared OAuth relay, and the OAuth state carries a CSRF token plus the app callback to return to (`{APP_URL}/auth/callback`).
...
1. The browser is redirected back to the app, now authenticated.
1. The frontend calls `/api/me` again and receives user info from the session.
